...
#
#     def book_maintainess(self,text,history,replies_dict,slot,slot_keys,new_slot,sys_intent):
#
#         if (SlotFilling.SlotFiller.search(text, replies_dict["trigger_dict"][
#                 slot_keys].split(","), slot_keys) is True):
#             return_sentence=False
#
#             if (len(replies_dict["need_ask_slot"]) >= 3) & (new_slot[slot_keys] is True):
#                 if len(history)==0:
#                     SlotFilling.SlotFiller.slotFilling(text, slot, slot_keys)
